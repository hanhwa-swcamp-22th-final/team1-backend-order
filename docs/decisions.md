...
## 2026-04-14 배송지 국가 코드 고정값 정리

### Decision
- 주문 서비스의 배송지 `country` 고정값은 `"USA"`가 아니라 `"US"`를 사용한다.

### Context
- 주문 서비스는 현재 미국 대상 배송만 처리하며, 배송지 `country`는 서버가 고정값으로 저장한다.
- 이 값은 이후 WMS 및 EasyPost 연동 데이터에도 전달된다.
- 기존 구현은 `"USA"`를 사용하고 있었지만, 연동 계약 기준은 `"US"`로 맞추는 편이 일관된다.

### Alternatives Considered
- 기존처럼 `"USA"`를 유지한다.
- 사용자 입력으로 `country`를 직접 받는다.
- 서버 고정값을 `"US"`로 통일한다.

### Why
- 사용자 입력 없이도 연동 필수값을 안정적으로 보장할 수 있다.
- 국가 코드를 `"US"`로 통일하면 응답 계약과 외부 연동 값 해석이 단순해진다.
