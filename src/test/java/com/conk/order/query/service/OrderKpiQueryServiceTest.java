...
class OrderKpiQueryServiceTest {
  ...
  /*
   * Mapper 가 반환한 각 상태별 건수가 FE 규격 필드로 올바르게 집계되는지 검증한다.
   * total=10, received=3, allocated=2, outboundInstructed=1,
   * picking=1, packing=1, outboundPending=0, outboundCompleted=2, canceled=1
   *
   * 기대값:
   *   todayTotal   = 10
   *   pendingCount = received(3) + allocated(2) + outboundInstructed(1) = 6
   *   pickingCount = picking(1) + packing(1) + outboundPending(0)       = 2
   *   shippedCount = outboundCompleted(2)
   */
  @Test
  void getKpi_assemblesResponseFromMapperCounts() {
    StubMapper stub = new StubMapper(10, 3, 2, 1, 1, 1, 0, 2, 1);
    OrderDashboardQueryService service = new OrderDashboardQueryService(
        ignoredOutboundStatsMapper(), stub, fixedClock());

    OrderKpiResponse response = service.getKpi(new OrderKpiQuery());

    assertThat(response.getTodayTotal()).isEqualTo(10);
    assertThat(response.getPendingCount()).isEqualTo(6);
    assertThat(response.getPickingCount()).isEqualTo(2);
    assertThat(response.getShippedCount()).isEqualTo(2);
  }
  ...
  void getKpi_returnsAllZero_whenNoOrders() {
    ...
    assertThat(response.getTodayTotal()).isZero();
    assertThat(response.getPendingCount()).isZero();
    assertThat(response.getPickingCount()).isZero();
    assertThat(response.getShippedCount()).isZero();
  }
  ...
  private Clock fixedClock() {
    return Clock.fixed(Instant.parse("2026-04-10T00:00:00Z"), ZoneOffset.UTC);
  }
}
